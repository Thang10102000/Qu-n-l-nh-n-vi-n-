...
function showInfo() {
    ...
    <tr>
    <th>${localStorage.getItem("FULL_NAME")}</th>
    </tr>
...
`)
    ...
    $('#info-department').append(`
<tr>
<th>${localStorage.getItem("department1")}</th>
</tr>

`)

    $('#info-position').append(`
<tr>
<th>${localStorage.getItem("position")}</th>
</tr>
...
`)
    ...
    $('#info-date').append(`
<tr>
<th>${localStorage.getItem("date")}</th>
</tr>
...
function showAccount() {
    ...
    for (var index = 0; index < listAccount.length; index++) {
        $('#Result_TB').append(`
              ...
              `)
        if (listAccount[index].AccountID == localStorage.getItem("ID")) {
            localStorage.setItem("department1", listAccount[index].Department);
            localStorage.setItem("position", listAccount[index].Position);
            localStorage.setItem("email", listAccount[index].Email);
            localStorage.setItem("date", listAccount[index].CreateDate);
        }
    }
...
function gotokhenthuong() {
    window.location.replace("khenthuong.html");
}
...
function showAvatar() {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, xhr) {
            ...
            let uri = "http://localhost:5501/Manager/Upload_IMG";
            $('.imgAvatar').append(`
            <img src="${uri}/${data}" alt="No Image" style="vertical-align: middle; width: 200px; height: 200px; border-radius: 50%;">`)
        },
        error(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR);
            console.log(textStatus);
...
//     tab = document.getElementById('Detail_Staff_ID');
...
function showAccountInfo() {
    let html = genAccountInfoPopup(currentAccount);
